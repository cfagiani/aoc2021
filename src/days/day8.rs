...
fn decode_output(digits: [Digit; 10], output: Vec<Digit>) -> u32 {
    let mut out_val = 0;
    for i in 0..4{
        let dig_idx = digits.iter().enumerate().find(|(_, v)| output.get(i).unwrap().eq(v)).unwrap();
...
fn decode_signal(signal: Vec<Digit>) -> [Digit; 10] {
    ...
    let three = find_and_remove(&mut digits, |d| d.segments.len() == 5 && d.segments.is_superset(&one.segments));

    // 6 has 6 segments and is the only 6 segment digit that doesn't have ALL of 1's segments
    let six = find_and_remove(&mut digits, |d| d.segments.len() == 6 && !d.segments.is_superset(&one.segments));
    ...
    let two = find_and_remove(&mut digits, |d| d.segments.len() == 5 && d.segments.contains(top_right));
...
fn parse_line(line: String) -> SignalPattern {
    ...
    return SignalPattern {
        digits: parts.get(0).unwrap().split(" ").map(|d| Digit::new(d)).collect(),
        result: parts.get(1).unwrap().split(" ").map(|d| Digit::new(d)).collect(),
    };
}
