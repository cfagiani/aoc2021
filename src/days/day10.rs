...
impl Day for Day10 {
    ...
    fn part1(&self, input_root: &str) {
        ...
        let corrupted_char: Vec<char> = lines.iter().map(|v| validate_lines(v, &char_pairs)).filter(|v| !v.0).map(|v| v.2.unwrap()).collect();
        let mut score = 0;
    ...
    fn part2(&self, input_root: &str) {
        let lines = get_data_from_file(input_root, "day10.txt", |s| s);
        let char_pairs = HashMap::<_, _>::from_iter(IntoIter::new([(')', '('), (']', '['), ('>', '<'), ('}', '{')]));
        let incomplete_lines: Vec<VecDeque<char>> = lines.iter().map(|v| validate_lines(v, &char_pairs)).filter(|v| v.0).map(|v| v.1).collect();
        let char_scores = HashMap::<_, _>::from_iter(IntoIter::new([('(', 1), ('[', 2), ('{', 3), ('<', 4)]));
        ...
        all_scores.sort();
        println!("Autocomplete score is {}", all_scores.get(all_scores.len() / 2).unwrap());
    }
}
...
fn validate_lines(line: &String, char_pairs: &HashMap<char, char>) -> (bool, VecDeque<char>, Option<char>) {
