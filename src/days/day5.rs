...
impl Day for Day5 {
    ...
    fn part1(&self, input_root: &str) {
        println!(
            "{} points crossed by more than one line",
            compute_crossed_lines(input_root, false)
        );
    }
    ...
    fn part2(&self, input_root: &str) {
        println!(
            "{} points crossed by more than one line",
            compute_crossed_lines(input_root, true)
        );
    }
}
...
fn compute_crossed_lines(input_root: &str, include_diagonal: bool) -> i32 {
    ...
    for line in lines {
        let x_step = if line.0.x == line.1.x {
            0
        ...
        let y_step = if line.0.y == line.1.y {
            0
...
fn parse_line(line: String) -> (Point, Point) {
    ...
    (
        point_from_string(parts.get(0).unwrap()),
        point_from_string(parts.get(1).unwrap()),
    )
}
