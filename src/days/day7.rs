...
impl Day for Day7 {
    ...
    fn part1(&self, input_root: &str) {
        ...
        println!("Total fuel used to move all crabs to {} is {}", destination, fuel_used);
    }
    ...
    fn part2(&self, input_root: &str) {
        ...
        println!("Total fuel used to move all crabs to {} is {}", destination, cost);
    }
}
...
fn parse(line: String) -> Vec<Crab> {
    return line.split(",").map(|v| Crab { pos: v.parse().unwrap() }).collect();
}
