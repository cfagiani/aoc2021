...
impl Day for Day9 {
    ...
    fn part1(&self, input_root: &str) {
        ...
        println!("Sum of risk levels is {}", low_points.iter().map(|p| p.val + 1).sum::<u32>());
    }
...
fn get_basin_size(height_map: &Vec<Vec<u32>>, low_point: &LowPoint) -> usize {
    ...
    while !horizon.is_empty() {
        ...
        let candidate_locs = get_candidates(&loc, height_map.len(), height_map.get(loc.i).unwrap().len());
...
fn get_candidates(loc: &Point, i_bound: usize, j_bound: usize) -> HashSet<Point> {
    ...
    if loc.i > 0 {
        candidates.insert(Point { i: loc.i - 1, j: loc.j });
    }
    if loc.i < i_bound - 1 {
        candidates.insert(Point { i: loc.i + 1, j: loc.j });
    }
    if loc.j > 0 {
        candidates.insert(Point { i: loc.i, j: loc.j - 1 });
    }
    if loc.j < j_bound - 1 {
        candidates.insert(Point { i: loc.i, j: loc.j + 1 });
    }
